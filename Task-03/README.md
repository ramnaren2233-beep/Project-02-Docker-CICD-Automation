...
| 3 | Secure Image Build | Built the secure Docker image successfully using `docker build`. | `03-secure-image-build.png` | ✅ Completed |
| 4 | Docker Image Verification | Verified `naren-kanban-secure:1.0` using Docker image listing. | `04-docker-image.png` | ✅ Completed |
| 5 | Container Deployment | Started the secure container on port `8080`. | `05-container-running.png` | ✅ Completed |
| 6 | Non-Root User | Configured the container to run as `appuser` instead of root. | `06-non-root-user.png` | ✅ Completed |
